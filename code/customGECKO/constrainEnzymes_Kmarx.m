 function [model,enzUsages,modifications,GAM,massCoverage] = constrainEnzymes_Kmarx(model,f,GAM,Ptot,pIDs,data,gRate,c_UptakeExp)
% constrainEnzymes
%
...
if sum(strcmp(model.rxns,'prot_pool_exchange')) == 0
    model = constrainPool_Kmarx(model,~measured,full(fs*Pbase));
end
if sum(data)==0
    %Modify protein/carb content and GAM:
    [model,GAM] = scaleBioMass_Kmarx(model,Ptot,GAM);
end
...
enzUsages = [];
if nargin > 7
    [tempModel,enzUsages,modifications] = flexibilizeProteins_altBranch(model,gRate,c_UptakeExp,c_source);
