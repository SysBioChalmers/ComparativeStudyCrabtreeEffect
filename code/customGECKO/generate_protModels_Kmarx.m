function generate_protModels_Kmarx(ecModel,grouping,name,ecModel_batch)
% generate_protModels
%
...
cd ../customGECKO
parameters = getModelParameters_Kmarx;
...
try
    movefile ../../../GECKO/databases/prot_abundance.txt ../../../GECKO/databases/prot_abundance_temp.txt
catch
...
[uniprotIDs,absValues,fermData,byProducts] = load_Prot_Ferm_Data_Kmarx(grouping);
...
for i=1:length(conditions)
    ...
    ecModelP  = changeMedia_batch_Kmarx(ecModel,c_source);
    tempModel = changeMedia_batch_Kmarx(ecModel_batch,c_source);
    %cd ../limit_proteins
    ...
    if Prot_diff>0
       [~,GAM] = scaleBioMass_Kmarx(tempModel,Ptot(i),[],true);
        ...
        %proteomics data
        ecModelP = scaleBioMass_Kmarx(ecModelP,Ptot(i),GAM,true);
        disp(' ')
    end
    ...
    tempModel       = setParam(tempModel,'lb',positionsEC(1),GUR*0.95);
    ...
    [ecModelP,usagesT,modificationsT,~,coverage] = constrainEnzymes_Kmarx(ecModelP,f,GAM,Ptot(i),pIDs,abundances,Drate(i),flexGUR);
...
%(for f factor calculation)
try
    movefile ../../../GECKO/databases/prot_abundance_temp.txt ../../../GECKO/databases/prot_abundance.txt
catch
