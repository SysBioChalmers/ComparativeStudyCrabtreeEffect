function [ecModel_batch,OptSigma] = getConstrainedModel_Kmarx(ecModel,modifications,name)
% getConstrainedModel
%
...
ecModel = changeMedia_batch_Kmarx(ecModel,c_source);
%Get a preliminary enzyme constrained model for performing the Kcats
%sensitivity analysis
[ecModel_batch,~,~] = constrainEnzymes_Kmarx(ecModel,f);
...
if ~isempty(solution.f)
    ...
    if (gRate-solution.x(ObjIndex))>0
        ...
        ecModel_batch = modifyKcats_Kmarx(ecModel_batch,gRate,modifications,name);
    else
        fprintf('\n')
        disp('***************************************************************')
